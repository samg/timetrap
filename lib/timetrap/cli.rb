  ...
  module CLI
    ...
    def edit
      entry = if args["-i"]
        ...
        Entry[args["-i"]]
      elsif Timer.active_entry
        warn "Editing running entry"
        Timer.active_entry
      elsif Timer.last_checkout
      ...
      entry.update start: args["-s"] if /.+/.match?(args["-s"])
      entry.update end: args["-e"] if /.+/.match?(args["-e"])
    ...
    def kill
      if e = Entry[args["-i"]]
        ...
        out << "(#{e.note}) " if /.+/.match?(e.note.to_s)
        if ask_user out
          e.destroy
          warn "it's dead"
        else
    ...
    def list
      sheets = ([Timer.current_sheet] | Entry.sheets).map do |sheet|
        ...
        if entries.empty?
          ...
          entries.each_with_object(sheet_atts) do |e, m|
            e_end = e.end_or_now
            m[:name] ||= sheet
      ...
      width = sheets.max_by { |h| h[:name].length }[:name].length + 4
      width = 10 if width < 10
      puts " %-#{width}s%-12s%-12s%s" % ["Timesheet", "Running", "Today", "Total Time"]
      sheets.each do |sheet|
        star = if sheet[:name] == Timer.current_sheet
          "*"
        else
          sheet[:name] == Timer.last_sheet ? "-" : " "
        end
    ...
    def now
      ...
      Timer.running_entries.each do |entry|
        ...
        out << " (#{entry.note})" if /.+/.match?(entry.note)
        puts out
      end
    end
